...
`policy_engine.py:43` 有明确 TODO：策略引擎当前只阻止未注册工具，风险等级、外部写入、删除能力等元数据**保留但不参与运行时判定**。前端也不渲染 `approval_required` 卡片。

**说法**："审批是个需要前后端 + 策略 + 测试 + 文档同步启用的横切功能，半接通比不接通更危险（用户以为有审批保护，实际没有）。所以我选择整条停用并显式标注，恢复时必须整体启用。"
